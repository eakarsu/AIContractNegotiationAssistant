/**
 * Custom Feature: regulatory-alerts
 * Regulatory change alerting
 *
 * POST /api/ai/regulatory-alerts
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_REGULATORY_ALERTS_KEY
...
const FEATURE_KEY = process.env.FEATURE_REGULATORY_ALERTS_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Regulatory change alerting.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIContractNegotiationAssistant - regulatory-alerts',
    },
    body: JSON.stringify({
...
router.post('/regulatory-alerts', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_REGULATORY_ALERTS_KEY');
    const ai = await callLLM({ feature: 'regulatory-alerts', goal: 'Regulatory change alerting', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'regulatory-alerts',
      endpoint: '/api/ai/regulatory-alerts',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[regulatory-alerts] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/regulatory-alerts/health', (req, res) => {
  res.json({
    feature: 'regulatory-alerts',
    endpoint: '/api/ai/regulatory-alerts',
