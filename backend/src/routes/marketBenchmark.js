/**
 * Custom Feature: market-benchmark
 * Market-based term benchmarking
 *
 * POST /api/ai/market-benchmark
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_MARKET_BENCHMARK_KEY
...
const FEATURE_KEY = process.env.FEATURE_MARKET_BENCHMARK_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Market-based term benchmarking.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIContractNegotiationAssistant - market-benchmark',
    },
    body: JSON.stringify({
...
router.post('/market-benchmark', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_MARKET_BENCHMARK_KEY');
    const ai = await callLLM({ feature: 'market-benchmark', goal: 'Market-based term benchmarking', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'market-benchmark',
      endpoint: '/api/ai/market-benchmark',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/market-benchmark/health', (req, res) => {
  res.json({
    feature: 'market-benchmark',
    endpoint: '/api/ai/market-benchmark',
